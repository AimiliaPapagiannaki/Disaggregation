...
def eliminate_offs(dl,off):
    ...
    up_thres_re = dl['rpwr'].quantile(.5)
        ...
        if i<= (df.shape[0]-2):
            ...
            if ((df['fridge'].iloc[i]==val) & (df['fridge'].iloc[i+1]==val)): #if consequtive 
                if i==0: # check first element
                    ...
                    deltaTime = (df.index[i+1]-df.index[i]).total_seconds()/60
                    div = round(deltaTime/cycle)
...
def main(argv):
    ...
    if ((datetime.datetime.now().weekday() == 0) | (datetime.datetime.now().weekday() == 3)):
    #if 0:
        print('Fixing parameters based on previous 3 days')
        ...
        end_ts = dt
        start_ts = end_ts + relativedelta(days= -3)
    ...
    # Create two columns for fridge on and fridge off and write to tb all on/off events
    dl['fr_on'] = np.nan
    dl['fr_off'] = np.nan
    dl.loc[dl['fridge']>0, 'fr_on'] = 1
    dl.loc[dl['fridge']<1, 'fr_off'] = 0
    
    fronoff = dl[['fr_on']].dropna().copy()
    fronoff['ts'] = fronoff.index
    fronoff['ts'] = fronoff.apply(lambda row: int(row['ts'].timestamp()) * 1000, axis=1)
    fronoff.set_index('ts', inplace=True, drop=True)
    mydict = fronoff.to_dict('index')
    #thingsio.send_data(address, devtoken, mydict)
    
    fronoff = dl[['fr_off']].dropna().copy()
    fronoff['ts'] = fronoff.index
    fronoff['ts'] = fronoff.apply(lambda row: int(row['ts'].timestamp()) * 1000, axis=1)
    fronoff.set_index('ts', inplace=True, drop=True)
    mydict = fronoff.to_dict('index')
    #thingsio.send_data(address, devtoken, mydict)
    dl = dl.drop(['fr_on','fr_off'], axis=1)
